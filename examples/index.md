...
**Executable-example coverage: 24/32 public labels (75%)** across 6 module program(s). The remaining labels carry no *executable* (`write … ; "expected"`, self-contained) example yet — closing that gap to 100% (with `@raises` error cases + sample data + live-VistA runs) is the Living Executable Examples roadmap (E2–E4).
...
| `VSLLOG` | 4 | 2 | [`VSLLOGEX.m`](programs/VSLLOGEX.m) |
