...
set(key,value) ; Set parameter `key` to `value` at the SYS entity; raise on a failed write.
 ...
 ; doc: HAZARD — XPAR treats "@" as the FileMan delete sentinel, so $$set(key,"@")
 ; doc: SILENTLY clears the SYS instance (equivalent to $$delete) instead of storing a
 ; doc: literal "@", and does NOT raise. A literal "@" payload cannot be round-tripped
 ; doc: through $$set. (Regression: tests/VSLCFGTST.m tSetAtSignSilentlyClears.)
