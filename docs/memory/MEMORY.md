...
- [vsl-wrapping-baseline-audit](vsl-wrapping-baseline-audit.md) — 2026-06-28 dual-engine adversarial audit → the wrapping baseline (`docs/proposals/vista-library-wrapping-baseline.md`) + four defects (VSLSEC default-duz UNDEF [✅ FIXED — `$$pduz($get(duz))`], VSLIO RED on IRIS [✅ FIXED — `write *-3` flush], VSLFS E-flag, VSLTASK "@"); green YDB suites masked them. Corrects [[m2-vslio]]. **VSLFS + VSLTASK still open.**
