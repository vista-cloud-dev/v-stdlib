VSLCFGTST ; v-stdlib — VSLCFG (XPAR config adapter) test suite.
 ...
 do tSetAtSignSilentlyClears(.pass,.fail)
 ;
...
tDeleteFailureIsLoud(pass,fail) ;@TEST "a failed $$delete maps to a clean ,U-VSL-CFG-DEL, $ECODE with the detail in $$lastError"
 ...
 quit
 ;
tSetAtSignSilentlyClears(pass,fail) ;@TEST "ADVERSARIAL (F2): $$set(key,""@"") SILENTLY clears the SYS instance (XPAR @ delete sentinel) — a literal @ payload is lost, NO raise"
 ; XPAR (like FileMan) treats "@" as the delete sentinel, so $$set(key,"@") clears the
 ; instance instead of storing a literal "@", and does NOT raise (unlike $$set of an
 ; undefined parameter). A caller storing a literal "@" loses it silently — see the
 ; VSLCFG $$set header note. Settles that this is the observed contract, not a bug to
 ; "fix" silently. Self-restoring (setup picks an already-empty SYS-settable param).
 new key
 do setup(.key)
 quit:key=""
 do set^VSLCFG(key,"realvalue")
 do eq^STDASSERT(.pass,.fail,$$get^VSLCFG(key,"MISS"),"realvalue","precondition: a real value is set at SYS")
 do set^VSLCFG(key,"@") ; must NOT raise — @ is the silent delete sentinel
 do eq^STDASSERT(.pass,.fail,$$get^VSLCFG(key,"CLEARED"),"CLEARED","$$set(key,""@"") silently cleared the instance (deleted, not stored as literal @)")
 do teardown(key)
 quit
 ;
 ; ---------- fixtures ----------
